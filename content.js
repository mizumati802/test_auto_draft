(() => {
  ...
  const Common = {
    ...
    fixedDelay: (ms) => new Promise(resolve => setTimeout(resolve, ms)),

    jitterDelay: (min, max) => {
      const ms = Math.floor(Math.random() * (max - min + 1)) + min;
      return Common.fixedDelay(ms);
    },
    ...
    Workflow: {
      ...
      start: async () => {
        ...
        if (resultA.count < 3 && resultA.hasImage) {
          ...
          await Common.fixedDelay(5000);
          Logic.Workflow.startBeta();
        } else {
      ...
      autoSaveScheduler: async () => {
        ...
        await Logic.Workflow.injectFinalData(Logic.Workflow.state.rootB);

        // 3. 注入後の安定時間を確保 (3秒)
        console.log("[Scheduler] Waiting for DOM stability (3s)...");
        await Common.fixedDelay(3000);

        // 4. 保存実行フェーズ (さらに2秒待機して確実性を高める)
        console.log("[Scheduler] Preparing to save (2s)...");
        await Common.jitterDelay(1500, 2500);
      ...
      injectFinalData: async (data) => {
        ...
        const titleEl = findEl(titleSelectors);
        if (titleEl) {
          titleEl.focus();
        ...
        const descEl = findEl(descSelectors);
        if (descEl) {
          descEl.focus();
          await Common.jitterDelay(1000, 3000);
        }
    ...
    vintage: (ctx) => {
      ...
      if (ctx.url.includes('/sell/draft/') && ctx.url.includes('auto_analyze')) {
        ...
        const startAutomation = async () => {
          ...
          await Common.fixedDelay(randomDelay);
